...
namespace nui {
	CDownloadVBox::CDownloadVBox(CMainFrameUI *p) :CSubVBox(p) {
		OutputDebugString(L"CDownloadVBox::CDownloadVBox ----->");
		m_container = dynamic_cast<ui::VBox*>(p->FindControl(L"download_box"));
		OutputDebugString(L"CDownloadVBox::CDownloadVBox <-----");
	}
	...
	*/
	void CDownloadVBox::CreateElementList(int count, int type) {
		...
		for (int i = 0; i < count; i++) {
			if (type == 0) {
				 element = buildSubListContainerElement(L"leigod/model/download_model_box.xml");
				...
				element = buildSubListContainerElement(L"leigod/model/downloaded_model_box.xml");
