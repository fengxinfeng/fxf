...
ui::VBox * CMainFrameUI::buildSubView(const wchar_t * xmlName)
{
...
bool CMainFrameUI::OnBtnAccrateStart(ui::EventArgs* args)
{
	...
	{
		CreateTipMsgBox(L"加速失败,请稍后重新加速");
	}
		...
		if (pMainOperatorListBox)
		{
			
			{
				...
				if (pGroupItem->vctGroupLines.size()/*groupData.stData.vctGroups.size()*/)
				{
					...
					/*std::sort(groupData.stData.vctGroups[0].vctGroupLines.begin(), groupData.stData.vctGroups[0].vctGroupLines.end(), [&](STLineItem & left, STLineItem & right) {
						return left.nDelay < right.nDelay;
					});*/
					/*				if (groupData.stData.vctGroups[0].vctGroupLines.size() > 6)
									{
										groupData.stData.vctGroups[0].vctGroupLines.erase(groupData.stData.vctGroups[0].vctGroupLines.begin() + 6, groupData.stData.vctGroups[0].vctGroupLines.end());
									}*/
...
{
	nbase::ThreadManager::PostTask(kThreadUI, ToWeakCallback([=]() {
		OnLineGroupResponse(m_nGameID, m_nAreaID);
	}));
