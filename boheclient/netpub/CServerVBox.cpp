...
namespace  nui {
	...
	CServerVBox::CServerVBox(CreateXmlObj *p):CSubVBox(p) {
		m_container = dynamic_cast<ui::VBox*>(p->FindControl(L"select_svr_area_node_vb"));
		if (m_container) {
			OutputDebugString(L"CServerVBox::CServerVBox  find self's parent select_svr_area_node_vb ");
		}
	};
	...
	void CServerVBox::CreateServerElementList(STGame &stGame) 
	{
		...
		ui::Button * pClose = dynamic_cast<ui::Button*>(FindSubControl(L"server_close_btn"));
		...
		if (pMainServerListBox)
		{
			...
			{
				ui::ListContainerElement * pElem = buildSubListContainerElement(L"bh/bh_list_mode_xml/server_model.xml");
				if (pElem)
				{
