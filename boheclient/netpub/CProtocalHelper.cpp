...
namespace leigod_netpub {
	...
	string GetFileNameOnUrlPath(const string & path);
	wstring GetDefaultLogoPath();
	wstring GetDefaultCountryPath();
...
//		wstring ws = CURLEncode::URLDecode(wsCntent);
	...
	bool CProtocalHelper::GetNationCodeList()
	{
		string jsonRes; 
		bool ret(false);
		//STCountryList CountryList;
		STCountryList &CountryList = BH_LOGIC_CENTER()->GetCountryCode();
		CURLcode err=curl_get_req_ex(NATIONCODE_URL,jsonRes,5);
		if (err == CURLE_OK && jsonRes.size())
		{
			if (UnPraseBase(jsonRes, CountryList))
			{
				int nCount = CountryList.vctTopCountry.size();
				
				for (int i = 0; i < nCount; i++)
				{
					STCountry & item = CountryList.vctTopCountry[i];
					if (item.ssIcoUrl.size())
					{
						item.ssImagePath = CountryPath(item.ssIcoUrl,1);
						if (!ImageValid(item.ssImagePath))
						{
							DownLoadFile(item.ssIcoUrl, item.ssImagePath);
							if (!ImageValid(item.ssImagePath))
							{
								item.ssImagePath = GetDefaultCountryPath();
							}
							ret = true;
						}
					}
				}
				nCount = CountryList.vctListCountry.size();
				for (int i = 0; i < nCount; i++)
				{
					STCountry & item = CountryList.vctListCountry[i];
					if (item.ssIcoUrl.size())
					{
						item.ssImagePath = CountryPath(item.ssIcoUrl, 0);
						if (!ImageValid(item.ssImagePath))
						{
							DownLoadFile(item.ssIcoUrl, item.ssImagePath);
							if (!ImageValid(item.ssImagePath))
	...
	wstring CProtocalHelper::CountryPath(string ssUrl, bool bChn)
	{
		wstring wsDir = ui::PathUtil::GetCurrentModuleDir() + L"gamelib\\country_pic\\top\\";
		if (!bChn)
		{
			wsDir = ui::PathUtil::GetCurrentModuleDir() + L"gamelib\\country_pic\\list\\";
		}
		string ssFileName = GetFileNameOnUrlPath(ssUrl);
		wstring wsDownPath = wsDir + U2W(ssFileName);
	...
	string GetFileNameOnUrlPath(const string & path)
	{
		...
		string ssName = sFileName;
		StringHelper::SplitString(ssName, '/', vSplittedString);
		if (nCount=vSplittedString.size() > 1)
		{
			return StringHelper::FormatA("%s%s", vSplittedString[nCount-1].c_str(), sExt);
		}
		return StringHelper::FormatA("%s%s", sFileName, sExt);
	}
...
{\
	...
	{
		wstring wsExepath = ui::PathUtil::GetCurrentModuleDir();
		return wsExepath + L"gamelib\\country_pic\\cn.png";
	}
